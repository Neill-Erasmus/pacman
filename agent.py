...
class Agent():
    def __init__(self, action_size: int) -> None:
        ...
        self.local_qnet = NeuralNetwork(action_size).to(self.device)
        self.target_qnet = NeuralNetwork(action_size).to(self.device)
        self.optimizer = optim.Adam(self.local_qnet.parameters(), lr=parameters.learning_rate)
    ...
    def step(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray, done: bool) -> None:
        ...
        if len(self.memory) > parameters.minibatch_size:
            experiences = random.sample(self.memory, k=parameters.minibatch_size)
            self.learn(experiences, parameters.gamma)

    def action(self, state: np.ndarray, epsilon: float = 0.) -> int:
        """
        ...
        state = preprocess_frame(state).to(self.device) # type: ignore
        ...
        if random.random() > epsilon:
            ...
            return int(random.choice(np.arange(self.action_size)))  # type: ignore
    ...
    def learn(self, experiences: List[Tuple[torch.Tensor, int, float, torch.Tensor, bool]], gamma: float) -> None:
        ...
        # Compute q_expected and q_targets
